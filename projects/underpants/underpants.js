...
_.each = function(aCollection, callback) {
    if (Array.isArray(aCollection)) {
        for (var i = 0; i < aCollection.length; i++) {
            callback(aCollection[i], i, aCollection);   
        }
        
    } else if (aCollection !== null && typeof aCollection === 'object') {
        for (var key in aCollection) {
            callback(aCollection[key], key, aCollection);  
        }
    }
};
